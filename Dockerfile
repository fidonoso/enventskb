...
RUN wait-for-it.sh srv-captain--dbkb:5432 -- mvn clean install -Dspring.profiles.active=test
...
ENTRYPOINT ["wait-for-it.sh", "srv-captain--dbkb:5432", "--", "java", "-jar", "target/eventsKB.jar"]
